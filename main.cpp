...
void FullScreen()
{
	HWND HMWB = FindWindow("WindowClass","SeewoFU");
	int cX = GetSystemMetrics(SM_CXSCREEN);   
	int cY = GetSystemMetrics(SM_CYSCREEN);
	SetWindowPos(HMWB,HWND_TOPMOST,0,0,cX,cY,SWP_SHOWWINDOW);
	SetWindowPos(HMWB,HWND_TOPMOST,0,0,cX,cY,SWP_SHOWWINDOW);
} 
...
void Minimize()
{
	...
	int cX = GetSystemMetrics(SM_CXSCREEN);   
	int cY = GetSystemMetrics(SM_CYSCREEN);
	SetWindowPos(HMWB,HWND_BOTTOM,0,0,cX,cY,SWP_HIDEWINDOW);
} 
...
void OnTrayIcon(HWND hWnd,LPARAM lParam)
{
	...
	AppendMenu(hmenu, MF_STRING, IDM_FULL, "全屏");
	AppendMenu(hmenu, MF_STRING, IDM_MIN, "最小化");
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	int cx = GetSystemMetrics(SM_CXSCREEN);   
	int cy = GetSystemMetrics(SM_CYSCREEN);
	HBITMAP bgbmp = (HBITMAP)LoadImageA(NULL,"bgb.bmp",IMAGE_BITMAP,cx,cy,LR_LOADFROMFILE|LR_DEFAULTSIZE|LR_CREATEDIBSECTION);
	wc.hbrBackground = CreatePatternBrush(bgbmp);
	...
	hwnd = CreateWindowEx(WS_EX_CLIENTEDGE,"WindowClass","SeewoFU",WS_VISIBLE|WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT, /* x */
		CW_USEDEFAULT, /* y */
		64, /* width */
		48, /* height */
	...
	if(hwnd == NULL) {
		...
		SetWindowPos(hwnd,HWND_BOTTOM,0,0,cx,cy,SWP_HIDEWINDOW);
